...
const OverviewContent: React.FC = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const [repeatedCustomers] = await Promise.all([
          authAxiosClient.get("/api/repeatedCustomers"),
        ]);
        console.log("repeatedCustomers:", repeatedCustomers?.data?.data);
        if (repeatedCustomers?.data?.data) {
          const res = repeatedCustomers?.data?.data;
          setCombinedData(res);
        }
      } catch (error) {
  ...
  return (
    ...
    >
      <Header title="ลูกค้าที่ซื้อบัตรซ้ำ" />
            ...
            <TableBody>
              {filteredEvents.map((event: any, eventIndex: number) => (
                ...
                >
                  ...
                  <TableCell sx={{ textAlign: "center", color: "black" }}>
                    {event.OrderCount
                      ? `${formatNumberWithCommas(event.OrderCount)}`
                      : "ยังไม่ระบุ"}
                  </TableCell>
                </TableRow>
              ))}
