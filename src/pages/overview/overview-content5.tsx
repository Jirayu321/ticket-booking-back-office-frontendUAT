    ...
    const fetchData = async () => {
      try {
        const [canceledOrders] = await Promise.all([
          authAxiosClient.get("/api/canceledOrders"),
        ]);
        console.log("canceledOrders:", canceledOrders?.data?.data);
        if (canceledOrders?.data?.data) {
          const res = canceledOrders?.data?.data;
          setCombinedData(res);
        }
      } catch (error) {
            ...
            <TableBody>
              {filteredEvents.map((event: any, eventIndex: number) => (
                ...
                >
                  ...
                  <TableCell sx={{ textAlign: "center", color: "black" }}>
                    {formatEventTime(event.Cancel_Date)}
                  </TableCell>
                  <TableCell sx={{ textAlign: "center", color: "black" }}>
                    {event.Remark_1}
                  </TableCell>
